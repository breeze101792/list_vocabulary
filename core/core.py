...
class Core(CommandLineInterface):
    def __init__(self, promote='PYD'):
        ...
        self.regist_cmd("mdict", operation.mdict, description="Memorize vocabulary.", arg_list = ['times', 'familiar', 'number', 'reverse'],  group='test')
        self.regist_cmd("forgotten", operation.forgotten, description="Memorize forgotten vocabulary.", arg_list = ['familiar', 'number'],  group='test')
